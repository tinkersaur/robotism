...
// Calibrate function. Take 100 readings (over 2 seconds) to calculate the gyro offset value.
// IMU should be steady in this process...
...
//  error = MPU6050_read (MPU6050_ACCEL_XOUT_H, (uint8_t *) &accel_t_gyro, sizeof(accel_t_gyro));
  if (error != 0) {
    Serial.print("MPU6050 Error xout_h:");
...
//  error = MPU6050_read(MPU6050_ACCEL_XOUT_H, (uint8_t *) &accel_t_gyro.reg.x_accel_h, 6);
  if (error != 0) {
    Serial.print("MPU6050 Error accel xout_h:");
    Serial.println(error);
  }
  // read X gyro
//  error = MPU6050_read(MPU6050_GYRO_XOUT_H, (uint8_t *) &accel_t_gyro.reg.x_gyro_h, 2);
  if (error != 0) {
    Serial.print("MPU6050 Error gyro xout_h:");
    Serial.println(error);
  }
...
//  error = MPU6050_read(MPU6050_INT_STATUS, &status, 1);
  if (error != 0) {
    Serial.print("MPU6050 Error int_status:");
    Serial.println(error);
  }
