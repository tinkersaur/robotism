...
class FineGrid(object):
    ...
    def reset(self):
        ...
        self.x0=-nCols*dx/2
        self.y0=-nCols*dy/2
        self.grid = [[ 1.0 for x in range(self.nCols)] for y in range(self.nRows)]    
    ...
    def enterRange (self,  carCumDist, carCurrAngle, scanDist, scanAngle):
        ...
        carX          = rangeXpos + carXpos
        carY          = rangeYpos + carYpos
        self.enterPoint (carX, carY)
    # end    
    ...
    def calcForceHist(self):
        ...
        for i in range(self.nRows):
            for j in range(self.nCols):
                rx = self.grid[i][j][0] - self.carX
                ry = self.grid[i][j][1] - self.carY
